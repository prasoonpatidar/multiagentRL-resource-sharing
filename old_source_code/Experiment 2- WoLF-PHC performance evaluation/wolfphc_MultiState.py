...
def wolfphc_MultiState(N,M,c,V,a,y_min,y_max,actionNumber):
    ...
    #卖家参数
    sellerActionSize = actionNumber  #卖家动作数
    ...
    timeLimit_min = 2 #timeLimit_min是以分钟为单位的限定时间
    t = -1
    while 1:
        #参数
        t += 1
    ...
    X = []
    for j in range(0,N):
        x_j = V * yAll[j] + a - np.e #x_j是np.array
        X.append(x_j)
    X = np.array(X)
    #返回七个性能指标值
    return pf.performance(X,yAll,c,V,a,N,M)
