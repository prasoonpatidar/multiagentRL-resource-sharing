...
from training.wolfPHC.run_helper import buyerPenaltiesCalculator, buyerUtilitiesCalculator, get_params
from training.wolfPHC.run_helper import logger_handle, initialize_agent, get_ys, choose_prob,\
    cumlativeBuyerExp, getPurchases, evaluation
...
def learn_policy(run_config, seller_info, buyer_info, train_config, logger_pass):
    ...
    for train_iter in range(0, train_config.iterations):
        ...
        seller_provided_resources = evaluation(sellers, train_config, all_seller_actions,
                                               ys, X,train_iter, seller_info, logger, train=True)
...
def eval_policy(seller_info, buyer_info, train_config, results_dir, logger_pass):
    ...
    for train_iter in range(0, train_config.iterations):
        ...
        seller_utilities, seller_penalties, \
        seller_provided_resources = evaluation(sellers, train_config,
                                               all_seller_actions, ys, X,train_iter, seller_info, logger, train=False)
